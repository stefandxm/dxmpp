...
void Connection::CheckStreamForFeatures()
{
    ...
    {
        Authentication = std::make_unique<SASL::SASL_Mechanism_EXTERNAL> (Client);
    ...
    {
        Authentication = std::make_unique<SASL::SASL_Mechanism_SCRAM_SHA1>(Client, MyJID, Password);
        Authentication->Begin();
        return;
    }
    if(FeaturesSASL_DigestMD5)
    {
        Authentication = std::make_unique<SASL::Weak::SASL_Mechanism_DigestMD5>( Client , MyJID, Password);
        Authentication->Begin();
        return;
    }
    if(FeaturesSASL_Plain)
    {
        Authentication = std::make_unique<SASL::Weak::SASL_Mechanism_PLAIN>( Client , MyJID, Password);
        Authentication->Begin();
        return;
    }
}
...
void Connection::Connect()
{
    ...
    Client = std::make_shared<Network::AsyncTCPXMLClient>(
                io_service,
                Certificate,
                Privatekey,
    ...
    // Fork io
    IOThread = std::make_unique<boost::thread>( boost::bind(&Connection::Run, this) );
}
